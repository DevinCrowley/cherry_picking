...
class Net(nn.Module):
    ...
    def normalize_state(self, state, update=True):
        ...
        if state.shape[-1] < self.welford_state_mean.shape[-1]:
            ...
            return (state - self.welford_state_mean[:state_size]) / sqrt(self.welford_state_mean_diff[:state_size] / self.welford_state_n)
        else:
    ...
    def denormalize_state(self, state):
        ...
        if state.shape[-1] < self.welford_state_mean.shape[-1]:
            ...
            return state * sqrt(self.welford_state_mean_diff / self.welford_state_n) + self.welford_state_mean
    ...
    def set_normalizer_stats(self, input_norm):
        welford_state_mean, welford_state_mean_diff, welford_state_n = input_norm
        self.welford_state_mean      = welford_state_mean
        self.welford_state_mean_diff = welford_state_mean_diff
        self.welford_state_n         = welford_state_n
